...
REMEDIATIONS: dict[str, dict] = {
    ...
    # ----------------------------------------------------- medicamentos_openfda
    "MED-01": _f(
        _d("NDC duplicado en el registro: el mismo código quedó listado más de una vez, típico de re-registros o transferencias entre razones sociales del mismo grupo.",
           "Duplicate NDC in the registry: the same code is listed more than once, typical of re-registrations or transfers between legal entities of the same group.",
           "NDC duplicado no registro: o mesmo código está listado mais de uma vez, típico de re-registros ou transferências entre razões sociais do mesmo grupo."),
        _d("Revisar los {n} códigos duplicados en «{col}» y quedarse con el registro vigente (el de fecha de listado más nueva), archivando el histórico.",
           "Review the {n} duplicate codes in “{col}” and keep the current record (the one with the newest listing date), archiving the historical one.",
           "Revisar os {n} códigos duplicados em «{col}» e manter o registro vigente (o de data de listagem mais nova), arquivando o histórico."),
        _d("Deduplicar por NDC al ingerir el catálogo (clave primaria en destino) y registrar el histórico en una tabla aparte de versiones.",
           "Deduplicate by NDC when ingesting the catalog (primary key at destination) and keep history in a separate versions table.",
           "Deduplicar por NDC ao ingerir o catálogo (chave primária no destino) e registrar o histórico numa tabela separada de versões."),
        _d("Equipo de Datos Regulatorios", "Regulatory Data Team", "Equipe de Dados Regulatórios")),
    "MED-02": _f(
        _d("Hoy sale 100%: todos los códigos respetan el formato NDC. Si baja, sería un error de tipeo o un cambio de formato en la fuente.",
           "Currently at 100%: every code follows the NDC format. If it drops, it would be a typo or a format change at the source.",
           "Hoje sai 100%: todos os códigos respeitam o formato NDC. Se cair, seria um erro de digitação ou uma mudança de formato na fonte."),
        _d("Corregir los {n} códigos de «{col}» que no respeten el patrón contra el registro oficial de la FDA.",
           "Fix the {n} codes in “{col}” that don't match the pattern against the official FDA registry.",
           "Corrigir os {n} códigos de «{col}» que não respeitem o padrão contra o registro oficial da FDA."),
        _d("Validar el patrón NDC (4-5 dígitos, guion, 3-4 dígitos) al ingerir, rechazando filas que no lo cumplan.",
           "Validate the NDC pattern (4-5 digits, hyphen, 3-4 digits) at ingestion, rejecting rows that don't comply.",
           "Validar o padrão NDC (4-5 dígitos, hífen, 3-4 dígitos) na ingestão, rejeitando linhas que não o cumpram."),
        _d("Equipo de Datos Regulatorios", "Regulatory Data Team", "Equipe de Dados Regulatórios")),
    "MED-03": _f(
        _d("Hoy sale 100%: todo producto declara su denominación común. Es el mínimo regulatorio — si baja, la fila no debería entrar al catálogo.",
           "Currently at 100%: every product declares its common name. That's the regulatory minimum — if it drops, the row shouldn't enter the catalog.",
           "Hoje sai 100%: todo produto declara sua denominação comum. É o mínimo regulatório — se cair, a linha não deveria entrar no catálogo."),
        _d("Completar el principio activo de las {n} filas sin «{col}» consultando el etiquetado (SPL) del producto.",
           "Fill in the active ingredient for the {n} rows missing “{col}” by checking the product's labeling (SPL).",
           "Completar o princípio ativo das {n} linhas sem «{col}» consultando a rotulagem (SPL) do produto."),
        _d("Hacer del principio activo un campo obligatorio del contrato de datos con la fuente.",
           "Make the active ingredient a mandatory field in the data contract with the source.",
           "Tornar o princípio ativo um campo obrigatório do contrato de dados com a fonte."),
        _d("Equipo de Datos Regulatorios", "Regulatory Data Team", "Equipe de Dados Regulatórios")),
    "MED-04": _f(
        _d("Medicamentos de uso humano terminados sin marca declarada en el registro — en general presentaciones institucionales o registros incompletos del laboratorio.",
           "Finished human-use drugs with no brand declared in the registry — usually institutional presentations or incomplete lab registrations.",
           "Medicamentos de uso humano acabados sem marca declarada no registro — em geral apresentações institucionais ou registros incompletos do laboratório."),
        _d("Revisar los {n} productos de uso humano sin «{col}»: si son genéricos puros, documentarlo como excepción de negocio; si no, completar la marca desde el etiquetado.",
           "Review the {n} human-use products missing “{col}”: if they're pure generics, document it as a business exception; otherwise fill in the brand from the labeling.",
           "Revisar os {n} produtos de uso humano sem «{col}»: se são genéricos puros, documentar como exceção de negócio; caso contrário, completar a marca a partir da rotulagem."),
        _d("Definir con Asuntos Regulatorios la regla de negocio: qué productos humanos pueden legítimamente no tener marca, y validar el resto en la ingesta.",
           "Define with Regulatory Affairs the business rule: which human products may legitimately lack a brand, and validate the rest at ingestion.",
           "Definir com Assuntos Regulatórios a regra de negócio: quais produtos humanos podem legitimamente não ter marca, e validar o resto na ingestão."),
        _d("Asuntos Regulatorios / Farmacovigilancia", "Regulatory Affairs / Pharmacovigilance", "Assuntos Regulatórios / Farmacovigilância")),
    "MED-05": _f(
        _d("Medicamentos de uso humano sin principios activos con concentración declarados — el dato existe en el etiquetado (SPL) pero no se cargó al NDC Directory.",
           "Human-use drugs without active ingredients with declared strength — the data exists in the labeling (SPL) but wasn't loaded into the NDC Directory.",
           "Medicamentos de uso humano sem princípios ativos com concentração declarados — o dado existe na rotulagem (SPL) mas não foi carregado no NDC Directory."),
        _d("Completar los principios activos de los {n} productos sin «{col}» cruzando con el etiquetado SPL del mismo NDC.",
           "Fill in the active ingredients for the {n} products missing “{col}” by cross-referencing the SPL labeling of the same NDC.",
           "Completar os princípios ativos dos {n} produtos sem «{col}» cruzando com a rotulagem SPL do mesmo NDC."),
        _d("Ingesta enriquecida: cruzar NDC Directory con el dataset de etiquetado (openFDA /drug/label) para completar automáticamente los faltantes.",
           "Enriched ingestion: cross the NDC Directory with the labeling dataset (openFDA /drug/label) to auto-fill the gaps.",
           "Ingestão enriquecida: cruzar o NDC Directory com o dataset de rotulagem (openFDA /drug/label) para completar automaticamente os faltantes."),
        _d("Equipo de Datos Regulatorios", "Regulatory Data Team", "Equipe de Dados Regulatórios")),
    "MED-06": _f(
        _d("~26% de los medicamentos de uso humano no declaran clase farmacológica — la FDA solo la exige en ciertos tipos de registro, pero para farmacovigilancia interna es un hueco real.",
           "~26% of human-use drugs don't declare a pharmacological class — the FDA only requires it for certain registration types, but for internal pharmacovigilance it's a real gap.",
           "~26% dos medicamentos de uso humano não declaram classe farmacológica — a FDA só a exige em certos tipos de registro, mas para a farmacovigilância interna é uma lacuna real."),
        _d("Clasificar los {n} productos sin «{col}» usando el principio activo como puente hacia una tabla de referencia de clases (ej. ATC de la OMS).",
           "Classify the {n} products missing “{col}” using the active ingredient as a bridge to a class reference table (e.g. WHO ATC).",
           "Classificar os {n} produtos sem «{col}» usando o princípio ativo como ponte para uma tabela de referência de classes (ex. ATC da OMS)."),
        _d("Mantener una tabla maestra principio activo → clase farmacológica, gobernada por Farmacovigilancia, que complete el dato en la ingesta.",
           "Maintain a master table active ingredient → pharmacological class, governed by Pharmacovigilance, that fills the field at ingestion.",
           "Manter uma tabela mestre princípio ativo → classe farmacológica, governada pela Farmacovigilância, que complete o dado na ingestão."),
        _d("Asuntos Regulatorios / Farmacovigilancia", "Regulatory Affairs / Pharmacovigilance", "Assuntos Regulatórios / Farmacovigilância")),
    "MED-07": _f(
        _d("Hoy sale 100%: todas las fechas de inicio de comercialización son YYYYMMDD válidas.",
           "Currently at 100%: all marketing start dates are valid YYYYMMDD.",
           "Hoje sai 100%: todas as datas de início de comercialização são YYYYMMDD válidas."),
        _d("Corregir las {n} fechas inválidas de «{col}» contra el expediente de registro del producto.",
           "Fix the {n} invalid dates in “{col}” against the product's registration file.",
           "Corrigir as {n} datas inválidas de «{col}» contra o processo de registro do produto."),
        _d("Validar el formato de fecha en la ingesta (YYYYMMDD estricto), rechazando valores no parseables.",
           "Validate the date format at ingestion (strict YYYYMMDD), rejecting unparseable values.",
           "Validar o formato de data na ingestão (YYYYMMDD estrito), rejeitando valores não parseáveis."),
        _d("Equipo de Datos Regulatorios", "Regulatory Data Team", "Equipe de Dados Regulatórios")),
    "MED-08": _f(
        _d("Productos terminados sin fecha de listado FDA vigente: el laboratorio no renovó (o no informó) el listado anual del producto.",
           "Finished products without a current FDA listing date: the lab didn't renew (or didn't report) the product's yearly listing.",
           "Produtos acabados sem data de listagem FDA vigente: o laboratório não renovou (ou não informou) a listagem anual do produto."),
        _d("Verificar los {n} productos con «{col}» faltante o vencida: confirmar con el laboratorio titular si siguen comercializándose o corresponde marcarlos discontinuados.",
           "Verify the {n} products with “{col}” missing or expired: confirm with the labeler whether they're still marketed or should be flagged discontinued.",
           "Verificar os {n} produtos com «{col}» faltante ou vencida: confirmar com o laboratório titular se continuam comercializados ou se devem ser marcados descontinuados."),
        _d("Alerta automática de puntualidad: revisar mensualmente los listados que vencen en los próximos 60 días.",
           "Automatic timeliness alert: review monthly the listings expiring within the next 60 days.",
           "Alerta automático de pontualidade: revisar mensalmente as listagens que vencem nos próximos 60 dias."),
        _d("Asuntos Regulatorios / Farmacovigilancia", "Regulatory Affairs / Pharmacovigilance", "Assuntos Regulatórios / Farmacovigilância")),
}
