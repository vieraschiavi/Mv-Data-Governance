...
| 🔷 **Power BI** | Gobiernos el modelo de Power BI en sí, no solo tus datos: apuntá a un proyecto `.pbip` (formato TMDL, offline, sin datos si borraste `cache.abf`) y el programa lee tablas, columnas, medidas (DAX), relaciones y roles RLS, y arma catálogo, glosario (cada medida = término), 5 reglas de salud del modelo y el **linaje real cableado de punta a punta: SQL Server detectado → tabla → dataset (modelo) → reporte**. Con tu API key opcional, pedí que la IA audite y refactorice el DAX de cualquier medida — se manda solo el DAX, nunca datos ([`docs/IA_EXTERNA.md`](docs/IA_EXTERNA.md)) |
