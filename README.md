...
| 🔷 **Power BI** | Gobiernos el modelo de Power BI en sí, no solo tus datos. **Modo offline**: apuntá a un proyecto `.pbip` (formato TMDL, sin datos si borraste `cache.abf`). **Modo tenant completo**: con tu propio service principal (opcional, apagado por defecto), escanea TODOS los workspaces vía la Scanner API de un clic. Cualquiera de los dos arma catálogo, glosario (cada medida = término), 5 reglas de salud del modelo y el **linaje cableado de punta a punta: SQL Server detectado → tabla → dataset (modelo) → reporte**. Con tu API key de IA opcional, pedí que audite y refactorice el DAX de cualquier medida — se manda solo el DAX, nunca datos ([`docs/BI_TENANT_SCAN.md`](docs/BI_TENANT_SCAN.md), [`docs/IA_EXTERNA.md`](docs/IA_EXTERNA.md)) |
| 📊 **Tableau** | El mismo gobierno del modelo, para Tableau: con tu propio Personal Access Token (opcional, apagado por defecto), escanea todo tu sitio vía la Metadata API — workbooks, datasources publicados, campos calculados y de dónde salen. Arma catálogo, glosario (cada campo calculado = término), 4 reglas de salud del sitio y linaje tabla de BD → datasource → workbook. Con tu API key de IA opcional, pedí que audite y refactorice cualquier fórmula calculada ([`docs/BI_TENANT_SCAN.md`](docs/BI_TENANT_SCAN.md)) |
