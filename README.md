...
| 🔎 **Mis datos** | Subí tu CSV/Excel **o conectate directo a tu base de datos** — 5 motores SQL clásicos (PostgreSQL, MySQL, SQL Server, Oracle, SQLite) **más los data warehouse/lake de nube más usados**: Snowflake, Google BigQuery, Databricks SQL Warehouse y Azure Synapse ([`docs/CLOUD_CONNECTORS.md`](docs/CLOUD_CONNECTORS.md)) y **Microsoft Fabric** — Lakehouse y Warehouse, con Entra ID, y además el gobierno **corriendo dentro de un notebook de Fabric** que deja las 9 tablas en el propio Lakehouse para que Power BI las lea nativo ([`docs/FABRIC.md`](docs/FABRIC.md)): perfil por columna, duplicados, PII detectada, y un **catálogo de calidad automático real** (`mvdg/auto_rules.py`) — reglas de completitud y unicidad generadas y evaluadas contra tu propio archivo, con puntaje 0-100 y estado pass/warn/fail, no solo texto sugerido. Sin registro, sin pedir acceso: subís el archivo y en minutos tenés el catálogo. Trae 4 **datasets de ejemplo reales, gobernados de punta a punta** (rotulado de alimentos, Uruguay · ventas de cafetería, Kaggle CC BY-SA 4.0 · campaña bancaria, UCI CC BY 4.0 · **medicamentos de 6 laboratorios multinacionales, openFDA dominio público — con defectos reales, no inyectados**): catálogo con dueño/steward, reglas de calidad con umbral, glosario y exportación/API lista para Power BI o Tableau |
| 📤 **BI & API** | Exportación CSV/Excel/JSON/Parquet + paquete Excel multi-hoja + API REST + **🔀 Migración a Purview y Collibra en las dos direcciones** — empuja catálogo/diccionario/glosario (acelerador, no reemplazo — el estado Draft/Approved o Candidate/Accepted sale de tu curaduría real, con qualifiedNames reales para Azure SQL Database) y **⬇️ trae** de Purview y de Collibra lo ya aprobado, con **💾 guardado local** que entra directo a 🖊️ Curaduría — + **🔒 Enforcement**: genera DDL de GRANT/REVOKE y enmascaramiento (PostgreSQL/SQL Server), nunca lo ejecuta + **🏷️ Etiquetas MIP** reales vía Microsoft Graph + **🔎 Escaneo batch** de tus conexiones + **☁️ Descubrimiento en Azure** (Resource Graph): inventaría todos los recursos de datos de tu suscripción en una consulta, sin agentes. Todo apagado por defecto, con modo previsualización sin credenciales, batching y reintento automático ante límites de tasa. Detalle honesto de qué se logró y dónde queda el techo — y un FAQ completo — en [`docs/PURVIEW_COLLIBRA.md`](docs/PURVIEW_COLLIBRA.md) y la pestaña ❓ Ayuda |
