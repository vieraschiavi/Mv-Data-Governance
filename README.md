...
| 📤 **BI & API** | Exportación CSV/Excel/JSON/Parquet + paquete Excel multi-hoja + API REST + **🔀 Migración a Purview/Collibra**: empuja catálogo, diccionario y glosario por API — acelerador, no reemplazo (el estado Draft/Approved de cada término sale de tu curaduría real). Modo previsualización sin credenciales; apagado por defecto. Detalle en [`docs/PURVIEW_COLLIBRA.md`](docs/PURVIEW_COLLIBRA.md) |
