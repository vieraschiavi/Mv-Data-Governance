...
| 📤 **BI & API** | Exportación CSV/Excel/JSON/Parquet + paquete Excel multi-hoja + API REST + **🔀 Migración a Purview/Collibra** (acelerador, no reemplazo — el estado Draft/Approved de cada término sale de tu curaduría real) + **🔒 Enforcement**: genera DDL de GRANT/REVOKE y enmascaramiento (PostgreSQL/SQL Server), nunca lo ejecuta + **🏷️ Etiquetas MIP**: aplica etiquetas reales vía Microsoft Graph a archivos en OneDrive/SharePoint + **🔎 Escaneo batch** de todas tus conexiones guardadas de un clic. Todo apagado por defecto, con modo previsualización sin credenciales. Detalle honesto de qué se logró y dónde queda el techo en [`docs/PURVIEW_COLLIBRA.md`](docs/PURVIEW_COLLIBRA.md) |
